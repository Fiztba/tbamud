...
/* =========================================================
 * Stubs needed when handler.c is in the build without interpreter.c
 *
 * Everything else handler.c reaches into -- fight.c, quest.c, mobact.c,
 * objsave.c, dg_scripts.c, mud_event.c, class.c and comm.c -- is stubbed in
 * the per-file sections above.  test_handler links lists.c for the group list
 * and nothing else, so one_argument has to come from here; test_handler_prefix
 * links the real interpreter.c, whose definition wins over this weak one.
 * ========================================================= */
