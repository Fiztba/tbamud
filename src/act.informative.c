...
int search_help(const char *argument, int level)
{
  int chk, bot, top, mid, minlen;
    ...
    * allocation.
    *
    * It is a hard crash, not a stray read. conf.h defines HAVE_STRNCASECMP,
    * so strn_cmp is strncasecmp, which has no NULL guard the way utils.c's
    * fallback does -- and the row past a calloc'd table has keywords ==
    * NULL. Plain builds SIGSEGV in strncasecmp just as sanitised ones do.
    *
