...
int get_line(FILE * fl, char *buf, size_t bufsize)
{
  ...
  do {
    if (!fgets(buf, bufsize, fl)) {
      /* On failure fgets() leaves buf untouched, so a caller that does not
       * check the return reads whatever the previous call left there.  An
       * error is worth a word; end of file is how this loop finishes. */
      if (ferror(fl))
